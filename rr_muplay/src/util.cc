...
bool is_exe_file(std::string file_path) {
  //checks access from user who invokes this not absolute if is executable
  if(! access(file_path.c_str(), X_OK))
    return true;
  return false;
}
/* Check if file path is elf file */
bool is_exe_elf_file(std::string file_path)
{
  MuplayElfReader elfReader(file_path);

  if(is_exe_file(file_path) && elfReader.is_elf_file())
    return true;
  return false;
}

/* Get all executable elfs in a directory */
std::set<std::string> get_all_exe_elfs(std::string dir_path) {
    std::set<std::string> res;
    if (!is_directory(dir_path.c_str())){
      //Not a directory returning nothing
      LOG(error) << "non directory passed to utils:get_all_exe_elfs... returning empty vector";
      return res;
    }
    for(auto& p : std::experimental::filesystem::recursive_directory_iterator(dir_path)) {
      std::experimental::filesystem::path exe_path;
      exe_path = is_symlink(p.path()) ? read_symlink(p.path()) : p.path();
      if(is_exe_elf_file(exe_path.c_str())) {
        //FOUND EXECUTABLE ELF FILE
        res.insert(exe_path);
      }
    }
    return res;
...
/* Pull the filename off the back of a path */

std::string get_filename(std::string path_string){
  std::experimental::filesystem::path path(path_string);
