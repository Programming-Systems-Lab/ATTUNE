...
/**
 * ANT EDIT TO EMULATE ABOVE BUT WITHOUT USING THE CURRENT STEP
 */
Completion ReplaySession::try_this_trace_step(
    ReplayTask* t, StepConstraints constraints,
    ReplayTraceStep this_step) {
  
  /* TODO Modify all of these to return GO_LIVE where appropriate */
  switch (this_step.action) {
    case TSTEP_RETIRE:
      return COMPLETE;
    case TSTEP_ENTER_SYSCALL:
      return enter_syscall(t, constraints);
...
ReplayResult ReplaySession::replay_step(const StepConstraints& constraints) {
  ...
  if (t) {
    ...
    if (!flags_.muplay_mode && has_deterministic_ticks(ev, current_step)) {
      check_ticks_consistency(t, ev);
    }
